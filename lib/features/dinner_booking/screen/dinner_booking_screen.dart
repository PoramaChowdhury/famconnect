...
class _AllUsersFreeTimeScreenState extends State<AllUsersFreeTimeScreen> {
  ...
  Future<void> _fetchAllUsersFreeSlots(DateTime day) async {
    ...
    // Define start and end for the selected day (to filter schedules by date)
    ...
    try {
      ...
      final futures = userIdNameMap.entries.map((entry) async {
        ...
        // Query schedules with status 'Free'
        ...
        // Filter schedules for selected day by comparing Timestamp to day range
        return schedulesSnapshot.docs.map((doc) {
          final data = doc.data();

          // Parse the Firestore Timestamp 'date' to DateTime
          final Timestamp timestamp = data['date'] as Timestamp;
          final DateTime date = timestamp.toDate();

          // Only include schedules for the selected date
          if (date.isBefore(dayStart) || !date.isBefore(dayEnd)) {
            // Not in selected date range, skip
            return null;
          }
